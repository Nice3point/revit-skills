...
description: >
  Supply Autodesk Revit API tests the documents, services, and data cases they run against, and pick the right data source for each situation.
  USE FOR: seeding a fresh in-memory model per test, opening installed sample .rvt/.rfa files as fixtures, choosing between [MethodDataSource] and [InstanceMethodDataSource], running one test body across several file kinds, injecting services under test through a dependency-injection data source, and skipping cleanly when a fixture or localization is absent.
...
[After(Test)]
[HookExecutor<RevitThreadExecutor>] // teardown touches Revit, so it runs on the Revit thread
